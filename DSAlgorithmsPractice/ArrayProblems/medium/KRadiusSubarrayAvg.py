...
def k_radius_avg_subarray(arr, k):
    if len(arr) < k:
        return arr
    output = [-1] * len(arr)

    for i in range(len(arr)):
        if i >= k and i+k < len(arr):
            start_index = i - k
            end_index = i + k
            total_elements = i - k + i + k + 1
            sum = sum+i for i in [start_index:end_index]
            output.insert(i, avg)

    print(output)


k_radius_avg_subarray([7,4,3,9,1,8,5,2,6], 3)
